...
import { timingSafeStringEqual } from '../timingSafeCompare.mjs'
import { isOriginAliasOfBoundRedirect } from './redirectOrigin.mjs'
...
export async function createAuthorizationCode(params) {
  ...
  const authCode = {
    ...
    redirect_uri,
    scope,
    nonce: nonce || null, // WHAT: Store nonce for OIDC ID token validation
...
export async function validateAndConsumeCode(params) {
  ...
  try {
    ...
    // Verify redirect_uri matches EXACTLY, with one narrow exception.
    // WHAT: A client that opted into origin preservation may have been redirected to a
    //       sibling origin of the callback it asked for (see lib/oauth/redirectOrigin.mjs).
    //       Such a client repeats the URI it *sent*, not the one it was redirected to.
    // WHY:  The exception is confined to two callbacks registered on this same client that
    //       differ only by origin. The code is already bound to the client, the client
    //       authenticates with its secret, and PKCE below binds the code to the browser
    //       that started the flow.
    if (authCode.redirect_uri !== redirect_uri) {
      const client = await getClient(client_id)
      const aliasAccepted = isOriginAliasOfBoundRedirect({
        client,
        boundRedirectUri: authCode.redirect_uri,
        presentedRedirectUri: redirect_uri,
      })
      ...
      logger.info('Authorization code redeemed against an origin-alias redirect_uri', {
        client_id,
        bound: authCode.redirect_uri,
    ...
    return {
      ...
      nonce: authCode.nonce, // Return nonce for ID token generation
    }
  } catch (error) {
