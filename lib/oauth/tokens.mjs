...
import { findUserByUUID } from './userLookup.mjs'
...
// WHY: Must resolve identically to the OIDC discovery document's issuer
//      (pages/api/.well-known/openid-configuration.js) — the two previously used
//      opposite precedence, so setting both vars to different values made
//      spec-compliant clients reject every token.
const JWT_ISSUER = process.env.JWT_ISSUER || getBaseUrl()
 ...
 * 
 * @param {Object} params
 * @param {string|null} [params.userId] - User UUID, or null/omitted for a
 *   client_credentials token that has no user context
 * @param {string} params.clientId - Client ID
 * @param {string} params.scope - Space-separated scopes
 * @param {string} [params.audience] - Resource the token is FOR (RFC 8707). Defaults to
 *   clientId, which is the caller itself - correct only when the resource server and the
 *   client are the same party.
...
export async function generateAccessToken(params) {
  const { userId = null, clientId, scope, audience, expiresIn = ACCESS_TOKEN_LIFETIME } = params

  // WHAT: clientId and scope are mandatory; userId is not.
  // WHY: the client_credentials grant is machine-to-machine and deliberately has no
  //      user context - pages/api/oauth/token.js passes userId: null. Requiring a
  //      truthy userId here made that grant throw on every request, which the token
  //      endpoint's catch turned into an HTTP 500 server_error.
  if (!clientId || !scope) {
    throw new Error('clientId and scope are required')
  }
  ...
  const payload = {
    ...
    // WHAT: `aud` names the RESOURCE the token is for, falling back to the caller's own
    //       client id when no resource was requested.
    // WHY: a resource server's only standards-conformant way to reject a token minted for
    //       somebody else is to check that it is in `aud` (RFC 9068 s4). With `aud` hard-wired
    //       to the caller, every token OpenClaw obtains reads `aud: <openclaw-client-id>`, so
    //       classscout cannot tell a token meant for it from one meant for salesleadgenerator
    //       and has to fall back to allow-listing caller ids. The default keeps every existing
    //       single-party token byte-identical.
  ...
  // WHAT: only user-bound tokens carry a `sub` claim.
  // WHY: validateAccessToken() maps a missing `sub` back to userId: null, which is what
  //      the permission routes already expect from a client_credentials token. Setting
  //      `sub` to the clientId instead would let a machine token satisfy a user-identity
  //      comparison (canReadOwnPermission) it must never satisfy.
  if (userId) {
    payload.sub = userId
  }
 ...
 * 
 * WHAT: Creates a JWT ID token with user identity claims per OpenID Connect spec
 * WHY: OAuth clients need user information to authenticate and personalize their service
 * HOW: Fetch user from either admin or public collection, encode claims as signed JWT
 * 
 * CRITICAL: Uses unified user lookup to support both admin and public users.
 * Previously only searched publicUsers collection, causing "User not found" errors for admin users.
 * 
 ...
 * @param {string} params.clientId - Client ID
 * @param {string} params.scope - Space-separated scopes
 * @param {string} [params.nonce] - OIDC nonce for replay attack prevention
 * @param {number} [params.expiresIn] - Token lifetime in seconds (default: 3600)
 * @returns {Promise<{token: string, jti: string, expiresAt: string}>}
 */
export async function generateIdToken(params) {
  const { userId, clientId, scope, nonce, expiresIn = ACCESS_TOKEN_LIFETIME } = params

  if (!userId || !clientId) {
    throw new Error('userId and clientId are required')
  }

  // WHAT: Fetch user data from either admin or public collection
  // WHY: OAuth needs to support both admin users (users collection) and public users (publicUsers collection)
  // HOW: Use unified lookup that checks both collections
  const user = await findUserByUUID(userId)
  if (!user) {
    logger.error('Failed to generate ID token: user not found', {
      userId,
      clientId,
  ...
  // WHAT: Build standard OIDC claims
  // WHY: OpenID Connect spec requires these claims for interoperability
  const payload = {
    iss: JWT_ISSUER,
    sub: userId,
    aud: clientId,
  ...
  // WHAT: Include nonce if provided (OIDC security)
  // WHY: Nonce prevents replay attacks by ensuring ID token is fresh
  if (nonce) {
    payload.nonce = nonce
  }

  // WHAT: Add claims based on requested scopes
  // WHY: Scopes control what user information is shared with the client
  ...
  if (scopes.includes('profile')) {
    ...
    // WHAT: Include user type and role in profile claims
    // WHY: Client apps may need to know if user is admin or public for authorization
    payload.user_type = user.type // 'admin' or 'public'
    payload.role = user.role // 'admin' or 'user'
    
    // WHAT: Include orgId for public users
    // WHY: Client apps may need organization context for multi-tenant features
    if (user.orgId) {
