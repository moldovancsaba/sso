...
function loadKeys() {
  ...
  try {
    ...
    PRIVATE_KEY = readFileSync(resolve(keysDir, 'private.pem'), 'utf8')
    PUBLIC_KEY = readFileSync(resolve(keysDir, 'public.pem'), 'utf8')
    return { PRIVATE_KEY, PUBLIC_KEY }
  } catch (error) {
    throw new Error(`Failed to load RSA keys: ${error.message}. Run: openssl genrsa -out keys/private.pem 2048 && openssl rsa -in keys/private.pem -pubout -out keys/public.pem`)
  }
}
