...
export async function updateClient(clientId, updates) {
  ...
  try {
    ...
    // DEBUG: Log the entire result structure safely
    try {
      logger.info('MongoDB findOneAndUpdate result structure', { 
        clientId,
        hasValue: !!result?.value,
        hasOk: !!result?.ok,
        hasLastErrorObject: !!result?.lastErrorObject,
        resultKeys: result ? Object.keys(result).filter(k => typeof result[k] !== 'function') : [],
        resultType: typeof result,
        hasClientId: !!(result?.client_id || result?.value?.client_id),
      })
    } catch (logErr) {
      logger.warn('Could not log result structure', { clientId })
    }

    // FIXED: MongoDB driver returns different structures depending on version
    // Modern driver (5.x+): result is the document directly (not wrapped)
    // Old driver (3.x/4.x): result has .value property
    // Check for the document by looking for common client fields
    let updatedClient
    if (result && result.client_id) {
      // Modern driver - result IS the document
      updatedClient = result
    } else if (result?.value) {
      // Old driver - result.value IS the document
    ...
    if (!updatedClient) {
      logger.error('Client not found or update failed', { 
        clientId, 
    ...
    logger.info('OAuth client updated successfully', { 
      clientId, 
