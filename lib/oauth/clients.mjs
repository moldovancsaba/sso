...
/**
 * classify - eligibility rules for the client_credentials (machine-to-machine) grant.
 *
 * WHAT: Decides whether an OAuth client may hold client_credentials.
 * WHY: The grant authenticates with client_secret alone - a public client
 *      (token_endpoint_auth_method: 'none') ships in a browser/mobile bundle, so any
 *      secret it holds is readable by anyone; a suspended or previously-excluded client
 *      must not silently regain a standing machine credential.
 *
 * Moved here (was previously defined only in scripts/enable-m2m-clients.mjs) so both the
 * one-off enablement script and the admin-UI update/registration path enforce the exact
 * same rule instead of two copies that could drift apart. The script re-exports this for
 * backward compatibility with its existing import path and test.
 *
 * @param {Object} client - fields checked: m2m_excluded, status, token_endpoint_auth_method, client_secret
 ...
 * @param {boolean} [clientData.preserve_initiating_origin] - Return the user to whichever
 *   registered domain they started on, rather than the one the app hardcoded into
 *   redirect_uri. Only for one application served on several domains. See
 *   lib/oauth/redirectOrigin.mjs.
 * @returns {Promise<{client: Object, client_secret: string}>} - Returns client object and plaintext secret (only time it's shown)
 */
/**
 * assertOriginPreservationIsMeaningful
 * WHAT: Rejects preserve_initiating_origin on a client whose redirect URIs all share one
 *       origin.
 * WHY:  On a single-origin client the setting can never do anything, so leaving it settable
 *       would let an operator believe multi-domain behaviour is configured when the client
 *       simply has nowhere else to send anyone.
 */
function assertOriginPreservationIsMeaningful(redirectUris) {
  const origins = new Set()
  for (const uri of redirectUris || []) {
    try {
      origins.add(new URL(uri).origin)
    } catch {
      // Malformed URIs are rejected by the dedicated validation above.
    }
  }

  if (origins.size < 2) {
    throw new Error(
      'preserve_initiating_origin requires redirect URIs on at least two different origins - it has no effect on a single-domain client',
    )
  }
}
...
export async function registerClient(clientData) {
  ...
  // WHAT: Redirect URIs are required only for grants that actually redirect a browser.
  // WHY: client_credentials has no user agent and no redirect leg (RFC 6749 s4.4), so a
  //      machine-only client has nowhere to redirect to. Demanding a URI here forced a
  //      fake one into the record, which would then be a usable redirect target on a
  //      client that must never participate in a browser flow.
  const REDIRECT_GRANTS = ['authorization_code', 'implicit']
  const usesRedirect = (grant_types || []).some((g) => REDIRECT_GRANTS.includes(g))
  if (usesRedirect && (!redirect_uris || redirect_uris.length === 0)) {
    throw new Error('At least one redirect URI is required')
  }
  if (!allowed_scopes || allowed_scopes.length === 0) {
    throw new Error('At least one scope is required')
  }
  // WHAT: Same eligibility rule as updateClient (see comment there) - checked against the
  // record as it will exist once created. status/client_secret aren't assigned yet at this
  // point in registration, so check the fields classify() actually needs directly rather
  // than building a partial fake record.
  if ((grant_types || []).includes('client_credentials') && (require_pkce || token_endpoint_auth_method === 'none')) {
    throw new Error('client_credentials cannot be enabled on a public client - it has no client_secret to authenticate with')
  }
  ...
  if (preserve_initiating_origin) {
    assertOriginPreservationIsMeaningful(redirect_uris)
  }
...
export async function updateClient(clientId, updates) {
  ...
  try {
    ...
    // WHAT: client_credentials authenticates with client_secret alone - reuse the same
    // eligibility rule scripts/enable-m2m-clients.mjs already enforces (and tests, in
    // __tests__/m2m-client-eligibility.test.js) rather than a second, drift-prone copy.
    // WHY: computed against the *effective* post-update record (grant_types is the only
    // field this check needs that a single PATCH could be changing).
    // Same "effective record" reasoning as the grant_types check below: either field can be
    // changing in this very PATCH, so both must be read post-update.
    const effectivePreserveOrigin = 'preserve_initiating_origin' in updateData
      ? updateData.preserve_initiating_origin
      : existingClient.preserve_initiating_origin
    if (effectivePreserveOrigin) {
      const effectiveRedirectUris = 'redirect_uris' in updateData
        ? updateData.redirect_uris
        : existingClient.redirect_uris
      assertOriginPreservationIsMeaningful(effectiveRedirectUris)
    }

    const effectiveGrantTypes = 'grant_types' in updateData ? updateData.grant_types : existingClient.grant_types
    if ((effectiveGrantTypes || []).includes('client_credentials')) {
      const { eligible, reason } = classify(existingClient)
      if (!eligible) {
        throw new Error(`client_credentials cannot be enabled on this client: ${reason}`)
      }
    }
