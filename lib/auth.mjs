...
export async function requireUnifiedAdmin(req, res) {
  ...
  // WHAT: Check if user has admin, super-admin, or owner role
  // WHY: After role system refactor, owner is the highest privilege level
  if (!['admin', 'super-admin', 'owner'].includes(result.adminPermission.role)) {
    res.status(403).json({
      error: {
        code: 'FORBIDDEN',
        message: 'Admin role required'
  ...
  return {
    ...
    permissions: ['owner', 'super-admin'].includes(result.adminPermission.role)
