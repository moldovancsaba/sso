...
import { validatePublicSession } from './publicSessions.mjs'
...
export async function getAdminPermission(userId) {
  try {
    ...
    // WHY: Handler needs role for authorization (admin vs super-admin)
    return {
      role: permission.role || 'admin', // 'admin' or 'super-admin'
...
export async function getPublicUserWithAdminCheck(req) {
  try {
    // WHAT: Validate public session cookie
    // WHY: All users (including admins) login via public auth system
    const sessionData = await validatePublicSession(req)
    ...
    const adminPermission = await getAdminPermission(sessionData.user.id)
...
export async function requireUnifiedAdmin(req, res) {
  ...
  // WHAT: Check if user has admin or super-admin role
  if (!['admin', 'super-admin'].includes(result.adminPermission.role)) {
    res.status(403).json({
      error: {
        code: 'FORBIDDEN',
        message: 'Admin role required'
  ...
  return {
    ...
    permissions: result.adminPermission.role === 'super-admin' 
      ? ['read', 'write', 'delete', 'manage-users', 'manage-orgs', 'manage-org-users']
      : ['read', 'write', 'manage-users'],
  }
}
