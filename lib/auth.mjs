...
import { isPublicSessionBoundToRequest } from './publicSessions.mjs'
import { validateRequestOrigin } from './middleware/csrf.mjs'

const COOKIE_NAME = process.env.ADMIN_SESSION_COOKIE || 'admin-session'
const DEFAULT_ADMIN_FRESH_AUTH_WINDOW_MS = parseInt(
  process.env.ADMIN_FRESH_AUTH_WINDOW_MS || `${15 * 60 * 1000}`,
  10
)
...
export async function getAdminUser(req) {
  ...
  // WHAT: Fetch user from database using the DB-verified session's userId
  // WHY: tokenData.userId is read from an unsigned, client-supplied cookie field —
  //      trusting it directly would let anyone with any valid session token swap in
  //      a different userId and be handed that user's identity. sessionValidation.session
  //      is the row the database actually associated with this token at creation time,
  //      so its userId is the one that can't be forged by editing the cookie.
  const user = await findUserById(sessionValidation.session.userId)
  if (!user) return null
...
// Removed: isAuthenticated(), hasPermission(), and requireAdmin(). All three gated on
// getAdminUser alone — the legacy `admin-session` cookie that the current OAuth admin login
// never issues — so any endpoint adopting them would have rejected every real admin. None had
// a caller. Use resolveAdminIdentity() below, or requireUnifiedAdmin() for API routes.
...
export async function getPublicUserWithAdminCheck(req) {
  try {
    ...
    const { getPublicSessionFromRequest } = await import('./publicSessions.mjs')
    const publicSession = await getPublicSessionFromRequest(req)
    ...
    const adminPermission = await getAdminPermission(publicSession.user.id)
 ...
 * WHAT: Resolves the current admin identity from either supported session model.
 * WHY: Two models are live at once — the OAuth admin login (/admin → /admin/callback →
 *      complete-oauth-login) issues only a `public-session` cookie backed by an
 *      appPermissions admin grant, while pre-OAuth logins issued `admin-session`. Callers
 *      that checked just one of them rejected sessions the other half of the app accepts:
 *      /admin/activity's getServerSideProps checked only `admin-session` and bounced every
 *      OAuth-logged-in admin back to /admin, which promptly redirected them to /admin/activity
 *      again. One resolver so page gates and API gates cannot disagree.
 *
 * @returns {Promise<{model: string, user: Object, session: Object|null}|null>}
 */
export async function resolveAdminIdentity(req) {
  const unified = await getPublicUserWithAdminCheck(req)

  if (unified?.adminPermission?.role === 'admin') {
    return {
      model: 'unified-public-session',
      session: unified.session || null,
      user: {
        id: unified.user.id,
        email: unified.user.email,
        name: unified.user.name,
        role: unified.adminPermission.role,
        permissions: ['read', 'write', 'delete', 'manage-users', 'manage-orgs', 'manage-org-users'],
      },
    }
  }

  const legacyAdmin = await getAdminUser(req)
  if (legacyAdmin) {
    return { model: 'legacy-admin-session', session: null, user: legacyAdmin }
  }

  return null
}

export function isFreshAuthenticationTimestamp(timestamp, windowMs = DEFAULT_ADMIN_FRESH_AUTH_WINDOW_MS, now = new Date()) {
...
export function hasBoundAdminSession(session, req) {
  return isPublicSessionBoundToRequest(session, req)
}
 ...
 */
export async function requireUnifiedAdmin(req, res, options = {}) {
  // WHAT: Reject cross-origin state-changing requests before touching the session
  // WHY: The public-session cookie is SameSite=None in production (required for cross-
  //      subdomain SSO), so it rides along on cross-site requests — cookie presence alone
  //      doesn't prove the request came from a page we trust.
  ...
  const freshAuthWindowMs = options.freshAuthWindowMs || DEFAULT_ADMIN_FRESH_AUTH_WINDOW_MS
  const authenticatedAt = result.session?.authenticatedAt || result.session?.createdAt || null
  const shouldRequireBoundSession = options.requireBoundSession ?? options.requireFreshAuth ?? false
  const hasBoundSession = hasBoundAdminSession(result.session, req)

  if (options.requireFreshAuth && !isFreshAuthenticationTimestamp(authenticatedAt, freshAuthWindowMs)) {
    res.status(403).json({
      error: {
        code: 'REAUTH_REQUIRED',
        message: 'Recent authentication required for this action. Please sign in again and retry.',
      }
    })
    return null
  }

  if (shouldRequireBoundSession && !hasBoundSession) {
    res.status(403).json({
      error: {
        code: 'REAUTH_REQUIRED',
        message: 'Session assurance is no longer sufficient for this action. Please sign in again and retry.',
      }
    })
    return null
  }
