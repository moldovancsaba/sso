...
/**
 * shouldTriggerPin
 * WHAT: Determines if PIN should be requested based on login count and system settings
 * WHY: Random between 5-10 logins to be unpredictable but not annoying
 * HOW: Checks both database settings and environment variable
 */
export async function shouldTriggerPin(loginCount) {
  // WHAT: Check environment variable first (highest priority)
  // WHY: Allow hard-disabling PIN for development/testing
  if (process.env.DISABLE_LOGIN_PIN === 'true') {
    return false
  }
  
  // WHAT: Check database settings for PIN verification toggle
  // WHY: Allow admins to enable or disable PIN verification without redeploying
  try {
    const db = await getDb()
    const settings = await db.collection(SETTINGS_COLLECTION).findOne({ _id: 'system' })
    
    if (settings && settings.pin_verification_enabled === false) {
      return false
    }
  } catch (error) {
    // WHAT: If database check fails, log and continue with default behavior
    // WHY: Don't block login if database is temporarily unavailable
    logger.warn('Failed to check PIN verification setting', { error: error.message })
  }
...
export async function verifyPin({ userId, pin, userType = 'admin' }) {
  ...
  // Verify PIN
  if (!timingSafeStringEqual(pinDoc.pin, pin)) {
    // Increment attempts
