...
export const roleSchema = z.preprocess(
  (role) => {
    ...
    return role.trim().toLowerCase() === 'super-admin' ? 'admin' : role.trim().toLowerCase()
  },
  z.enum(['admin'], {
...
function normalizePermissionRoleValue(role) {
  if (typeof role !== 'string') return role

  const normalized = role.trim().toLowerCase()
  if (normalized === 'guest') return 'none'
  if (normalized === 'owner' || normalized === 'superadmin' || normalized === 'super-admin') return 'admin'
  return normalized
}

// Public/app permission role validation
// WHAT: Canonical three-role system with legacy alias normalization
// WHY: Runtime permission APIs are being standardized on none/user/admin
export const publicRoleSchema = z.preprocess(
  normalizePermissionRoleValue,
  z.enum(['none', 'user', 'admin'], {
    errorMap: () => ({ message: 'Role must be "none", "user", or "admin"' }),
  })
)
 ...
 * WHY: Prevent accidental privilege escalation and preserve safeguards around legacy owner-tagged records
 */

/**
 * isOwnerRole
 * WHAT: Check if a role is 'owner'
 * WHY: Legacy owner-tagged records remain immutable and require special protection
 * 
 * @param {string} role - Role to check
 * @returns {boolean} True if role is 'owner'
 */
export function isOwnerRole(role) {
  return normalizePermissionRoleValue(role) === 'admin' && String(role).trim().toLowerCase() === 'owner'
}

/**
 * canChangeRole
 * WHAT: Validates if an actor can change a target user's role
 * WHY: Prevent unauthorized role changes and protect legacy owner-tagged records
 * 
...
export function canChangeRole(actorRole, currentRole, newRole) {
  const normalizedActorRole = normalizePermissionRoleValue(actorRole)
  const normalizedCurrentRole = normalizePermissionRoleValue(currentRole)
  const normalizedNewRole = normalizePermissionRoleValue(newRole)

  if (!['none', 'user', 'admin'].includes(normalizedActorRole)) {
    return { allowed: false, reason: 'Invalid actor role.' }
  }

  if (!['none', 'user', 'admin'].includes(normalizedCurrentRole)) {
    return { allowed: false, reason: 'Invalid current role.' }
  }

  if (!['none', 'user', 'admin'].includes(normalizedNewRole)) {
...
export function getRoleHierarchyLevel(role) {
  ...
  return levels[normalizePermissionRoleValue(role)] ?? 0
}
