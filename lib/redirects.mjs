...
export function isSafeRedirectTarget(target, options = {}) {
  ...
  // WHAT: '/' alone is a safe relative path, but '//host/path' is browser-interpreted as
  //       protocol-relative — i.e. an external redirect to whatever origin follows the slashes.
  // WHY: Without excluding it, isSafeRedirectTarget('//evil.com') would incorrectly return true.
  if (allowRelative && normalizedTarget.startsWith('/') && !normalizedTarget.startsWith('//')) {
    return true
  }
