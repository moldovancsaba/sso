...
/**
 * WHAT: Ensure a public user has a stable UUID identifier.
 * WHY: Legacy records may still only have MongoDB ObjectId, but sessions and
 *      OAuth flows expect UUID-based user IDs consistently.
 *
 * @param {Object} user - Public user document
 * @returns {Promise<Object>} Same user object with `id` guaranteed
 */
export async function ensurePublicUserId(user) {
  if (!user) {
  ...
  if (!user._id) {
    throw new Error('Cannot assign UUID without user _id')
  }
  ...
  await usersCollection.updateOne(
    { _id: user._id },
    { $set: { id: uuid, updatedAt: new Date().toISOString() } }
  )
 ...
 */
/**
 * WHAT: Update public user password
 * WHY: Enable password changes for forgot password and password reset flows
 * 
 * @param {string} userId - User UUID
 * @param {string} newPassword - New password (plain text, will be hashed)
 * @returns {Promise<void>}
 */
export async function updatePublicUserPassword(userId, newPassword) {
  const db = await getDb()
  const usersCollection = db.collection('publicUsers')
  ...
  // WHAT: Hash new password with bcrypt
  ...
  logger.info('Public user password updated', {
    userId,
