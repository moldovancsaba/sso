 ...
 * @param {string} params.role - User role: 'guest' | 'user' | 'admin' | 'owner'
 ...
 */
export async function updateAppPermission({ userId, clientId, hasAccess, status, role, grantedBy, actorRole }) {
  await ensureIndexes()
  const db = await getDb()
  const col = db.collection('appPermissions')
