...
import { canChangeRole } from './validation.mjs'

export const CANONICAL_APP_PERMISSION_ROLES = ['none', 'user', 'admin']
export const CANONICAL_APP_PERMISSION_STATUSES = ['pending', 'approved', 'revoked']
...
export function normalizePermissionRole(role) {
  if (!role || typeof role !== 'string') return 'none'

  const normalized = role.trim().toLowerCase()

  if (normalized === 'user' || normalized === 'admin' || normalized === 'none') {
    return normalized
  }

  if (normalized === 'guest') return 'none'
  if (normalized === 'owner' || normalized === 'superadmin' || normalized === 'super-admin') {
    return 'admin'
  }
...
export function normalizePermissionStatus(status) {
  if (!status || typeof status !== 'string') return 'revoked'

  const normalized = status.trim().toLowerCase()

  if (normalized === 'pending' || normalized === 'approved' || normalized === 'revoked') {
    return normalized
  }
...
export function isValidPermissionRole(role) {
  return CANONICAL_APP_PERMISSION_ROLES.includes(normalizePermissionRole(role))
}

export function isValidPermissionStatus(status) {
  return CANONICAL_APP_PERMISSION_STATUSES.includes(normalizePermissionStatus(status))
}

export function permissionHasAccess(status, role, explicitHasAccess = null) {
  const normalizedStatus = normalizePermissionStatus(status)
  const normalizedRole = normalizePermissionRole(role)

  if (typeof explicitHasAccess === 'boolean') {
    return explicitHasAccess && normalizedStatus === 'approved' && normalizedRole !== 'none'
  }

  return normalizedStatus === 'approved' && normalizedRole !== 'none'
}

export function normalizePermissionRecord(permission) {
  if (!permission) return null

  const role = normalizePermissionRole(permission.role)
  ...
  return {
    ...
    role,
    status,
    hasAccess: permissionHasAccess(status, role, permission.hasAccess),
  }
}
...
export async function getAppPermission(userId, clientId) {
  ...
  const permission = normalizePermissionRecord(await col.findOne({ userId, clientId }))
...
export async function createAppPermission({ userId, clientId, appName, autoApprove = false }) {
  ...
  return normalizePermissionRecord(await col.findOne({ userId, clientId }))
}

/**
 * updateAppPermission
 * WHAT: Updates user's permission for an app (grant/revoke access, change role)
 * WHY: Used by app admins and SSO admins to manage user access
 * 
 * @param {Object} params
 * @param {string} params.userId - User's SSO UUID
 * @param {string} params.clientId - OAuth client ID
 * @param {boolean} params.hasAccess - Whether user has access
 * @param {string} params.status - Permission status: 'approved' | 'pending' | 'revoked'
 * @param {string} params.role - User role: 'none' | 'user' | 'admin'
 * @param {string} params.grantedBy - Admin who granted/revoked access
 * @param {string} params.actorRole - Role of admin making the change (for validation)
 * @param {string} params.appName - Optional app display name for create/update flows
 * @returns {Promise<Object>} Updated permission record
 */
export async function updateAppPermission({ userId, clientId, hasAccess, status, role, grantedBy, actorRole, appName = null }) {
  await ensureIndexes()
  const db = await getDb()
  const col = db.collection('appPermissions')
  ...
  // WHAT: Get current permission to validate role change
  // WHY: Need to preserve protection around legacy owner-tagged records
  const currentPermission = await col.findOne({ userId, clientId })
  const currentRole = normalizePermissionRole(currentPermission?.role || 'none')
  
  // WHAT: Validate role change using protection rules
  // WHY: Prevent unauthorized role changes and protect legacy owner records
  if (normalizedRole && normalizedRole !== currentRole) {
    const validation = canChangeRole(actorRole || 'admin', currentRole, normalizedRole)
    if (!validation.allowed) {
      logger.warn('Role change blocked', {
        userId,
        clientId,
        currentRole,
  ...
  if (normalizedStatus === 'approved' && normalizedHasAccess) {
  ...
  if (normalizedStatus === 'revoked' || !normalizedHasAccess) {
  ...
  const insertDefaults = {
    userId,
    clientId,
    appName: appName || currentPermission?.appName || null,
    requestedAt: currentPermission?.requestedAt || now,
  ...
  return normalizePermissionRecord(result)
}
...
export async function getUserAppPermissions(userId) {
  ...
  const permissions = (await col
    .find({ userId })
    .sort({ appName: 1 })
    .toArray()).map(normalizePermissionRecord)
 ...
 * @param {string} filters.status - Filter by status: 'pending' | 'approved' | 'revoked'
...
export async function getAppUsers(clientId, filters = {}) {
  ...
  if (filters.status) {
    query.status = normalizePermissionStatus(filters.status)
  }
...
export async function upsertPermissionForAdmin({ userId, clientId, role, status = 'approved', adminUserId }) {
  ...
  if (existing) {
    ...
    const update = {
      role: normalizedRole,
      status: normalizedStatus,
      hasAccess: permissionHasAccess(normalizedStatus, normalizedRole),
      updatedAt: now,
    }
    
    // WHAT: Set grantedAt/grantedBy if moving to approved status
    // WHY: Track who approved and when for audit purposes
    if (normalizedStatus === 'approved' && normalizePermissionStatus(existing.status) !== 'approved') {
      update.grantedAt = now
    ...
    logger.info('App permission updated by admin', {
      ...
      status: normalizedStatus,
      adminUserId,
      wasApproved: normalizePermissionStatus(existing.status) === 'approved',
    })
    ...
    const permission = {
      ...
      grantedAt: normalizedStatus === 'approved' ? now : null,
      grantedBy: normalizedStatus === 'approved' ? adminUserId : null,
      revokedAt: null,
      revokedBy: null,
      lastAccessedAt: null,
      createdAt: now,
...
export function mapPermissionToDTO(permission) {
  ...
  return {
    ...
    role: normalized.role || 'none',
    status: normalized.status || 'revoked',
    hasAccess: normalized.hasAccess || false,
    requestedAt: normalized.requestedAt || null,
    grantedAt: normalized.grantedAt || null,
    grantedBy: normalized.grantedBy || null,
    revokedAt: normalized.revokedAt || null,
    revokedBy: normalized.revokedBy || null,
    lastAccessedAt: normalized.lastAccessedAt || null,
