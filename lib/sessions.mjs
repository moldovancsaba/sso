...
import { normalizeAdminRole } from './users.mjs'
 ...
 * WHAT: Generates a unique fingerprint for the client device
 * WHY: Track sessions by device; detect suspicious logins from new devices
 * HOW: Hash combination of IP address and User-Agent
 * 
 * @param {object} metadata - Request metadata (ip, userAgent)
 * @returns {string} Device fingerprint hash
 */
function generateDeviceFingerprint(metadata) {
  const components = [
    metadata.ip || 'unknown',
...
export async function createSession(userId, email, role, maxAgeSeconds = 7 * 24 * 60 * 60, metadata = {}) {
  ...
  const deviceFingerprint = generateDeviceFingerprint(metadata)
  ...
  const session = {
    ...
    deviceFingerprint, // Track device for security monitoring
    revokedAt: null,
    revokeReason: null,
  }
...
export async function validateSession(token) {
  ...
  // WHY: Security hardening - admin sessions expire after 4 hours of inactivity
  // NOTE: Changed from 30 days to 4 hours for better security (Phase 4 hardening)
  const sessionLifetimeMs = 4 * 60 * 60 * 1000 // 4 hours in milliseconds
  const newExpiresAt = new Date(now.getTime() + sessionLifetimeMs)
...
export async function getUserActiveSessions(userId) {
  ...
  return sessions.map((s) => ({
    ...
    deviceFingerprint: s.deviceFingerprint, // Include for device tracking
  }))
}

/**
 * checkDeviceChange
 * WHAT: Checks if current request is from a different device than previous sessions
 * WHY: Detect suspicious logins from new devices; alert admins of potential compromise
 * HOW: Compare current device fingerprint with user's recent sessions
 * 
 * @param {string} userId - UUID of user
 * @param {object} metadata - Current request metadata (ip, userAgent)
 * @returns {Promise<{isNewDevice: boolean, recentDevices: Array}>}
 */
export async function checkDeviceChange(userId, metadata) {
  const col = await getSessionsCollection()
  const now = new Date()
  ...
  if (recentSessions.length === 0) {
    // First login ever or no recent sessions
    return { isNewDevice: false, recentDevices: [] }
  }
  
  const currentFingerprint = generateDeviceFingerprint(metadata)
  const recentDevices = [...new Set(recentSessions.map(s => s.deviceFingerprint))]
  const isNewDevice = !recentDevices.includes(currentFingerprint)
