...
function requireFacebookConfig({ requireSecret = false, redirectUri = null } = {}) {
  if (!FACEBOOK_APP_ID) {
    throw new Error('FACEBOOK_APP_ID environment variable is not set')
  }

  if (requireSecret && !FACEBOOK_APP_SECRET) {
 ...
 */
export function getFacebookAuthUrl(csrfToken, oauthRequest = null, redirectUri = null) {
  requireFacebookConfig({ redirectUri })
...
export async function exchangeCodeForToken(code, redirectUri = null) {
  requireFacebookConfig({ requireSecret: true, redirectUri })
