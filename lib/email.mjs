 ...
 */
// WHAT: One pooled SMTP transport per distinct credential set, cached at module scope.
// WHY: Two separate problems, and only doing both fixes anything.
//
//      Reuse alone does not reduce connections. Measured against a local SMTP sink,
//      five messages cost five TCP connections whether a fresh transport was built per
//      send or one transport was reused — a non-pooled nodemailer transport opens a new
//      connection for every sendMail. Only `pool: true` changes that: the same five
//      messages then cost one connection. But a pool is pointless if the transport
//      holding it is discarded after each send, so the cache is what makes pooling
//      reachable, not an optimisation in its own right.
//
//      Keyed rather than a single instance because sendViaNodemailer() accepts a
//      per-organisation emailConfig (see lib/orgEmailConfig.mjs). A lone global
//      singleton would silently route organisation mail through the default account.
//
//      Safe on serverless despite frozen instances: verified that when a peer drops an
//      idle pooled socket, the next send transparently opens a replacement rather than
//      failing — three sends across a forced mid-flight disconnect all succeeded, on two
//      connections, with no error surfaced.
//
//      One consequence to know about: a pooled connection is an open handle, so it keeps
//      the event loop alive. That is irrelevant inside an API route, where the response
//      ends the invocation, but a plain Node script that imports this module will not
//      exit on its own once mail has been sent. Both current script callers
//      (scripts/test-email-config.mjs, scripts/test-magic-link.mjs) already call
//      process.exit() explicitly, so none hangs today; a new one must do the same, or
//      close the transports it used.
const nodemailerTransporters = new Map()

// WHAT: Cache key derived from the full credential set, hashed.
// WHY: Two organisations can share a host and username while differing in password, so
//      the password has to participate in identity — but it must never sit in a Map key
//      that could surface in a heap dump or a debug log. A digest gives identity without
//      retaining the secret.
function transporterCacheKey(smtp) {
  return createHash('sha256')
    .update([smtp.host, smtp.port, smtp.secure, smtp.user, smtp.pass].join('\u0000'))
    .digest('hex')
}

// WHAT: Build-or-reuse a pooled transport for one SMTP configuration.
// WHY: Single place where pool settings are defined, so verify() exercises exactly the
//      transport that sends use. maxConnections is deliberately small: the benefit here
//      is collapsing a burst of magic links onto one connection, not parallelism, and
//      providers cap concurrent connections per account.
function getPooledTransporter(smtp) {
  const key = transporterCacheKey(smtp)
  const existing = nodemailerTransporters.get(key)
  ...
  nodemailerTransporters.set(key, transporter)
  return transporter
}
...
function getNodemailerTransporter() {
  ...
  // WHAT: Resolve through the same cache the send path uses.
  // WHY: This function backs validateEmailConfig()'s transporter.verify(). It previously
  //      built its own separate transport, so a successful verify() proved nothing about
  //      the connection sends would actually make.
  return getPooledTransporter(smtp)
}
...
async function sendViaNodemailer({ to, subject, text, emailConfig }) {
  ...
  const transporter = getPooledTransporter(cfg.smtp)
