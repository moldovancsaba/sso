...
/**
 * revokeAllSessions
 * WHAT: Ends the caller's session under both supported models. Does not navigate.
 * WHY: Two session models are live at once — see resolveAdminIdentity in lib/auth.mjs. Signing
 *      out of one leaves the other valid, which is not a logout. Every logout path in the app
 *      calls this so none of them can cover only half again: the admin pages used to clear only
 *      `admin-session` (a cookie the OAuth admin login never sets), and /logout used to pair the
 *      public call with `/api/users/logout`, which has been a 410 tombstone for several releases.
 *      Calling the endpoint for a model you are not using is harmless — it clears a cookie you
 *      do not have and returns 200.
 *
 * @throws {Error} only when *no* endpoint could be reached at all.
 */
export async function revokeAllSessions() {
  const results = await Promise.allSettled([
    fetch('/api/public/logout', { method: 'POST', credentials: 'include' }),
    fetch('/api/admin/login', { method: 'DELETE', credentials: 'include' }),
  ])

  // Only a transport failure on *both* means nothing was revoked; either endpoint answering
  // (even with an error status) has already cleared its cookie.
...
/**
 * logoutAdmin
 * WHAT: revokeAllSessions, then back to the admin entry point.
 */
