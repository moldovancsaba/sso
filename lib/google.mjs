...
import { buildOAuthCallbackState } from './oauth/callbackState.mjs'

const GOOGLE_CLIENT_ID = process.env.GOOGLE_CLIENT_ID
const GOOGLE_CLIENT_SECRET = process.env.GOOGLE_CLIENT_SECRET
const GOOGLE_REDIRECT_URI = process.env.GOOGLE_REDIRECT_URI

function requireGoogleConfig({ requireSecret = false, redirectUri = null } = {}) {
  if (!GOOGLE_CLIENT_ID) {
    throw new Error('GOOGLE_CLIENT_ID environment variable is not set')
  }

  if (requireSecret && !GOOGLE_CLIENT_SECRET) {
 ...
 * @param {string} redirectUri - Override redirect URI (for localhost testing)
 * @param {{ prompt?: string, loginHint?: string }} options - Optional Google auth parameters
 * @returns {string} Google authorization URL
 */
export function getGoogleAuthUrl(csrfToken, oauthRequest = null, redirectUri = null, options = {}) {
  requireGoogleConfig({ redirectUri })

  const stateParam = buildOAuthCallbackState({
    csrfToken,
    oauthRequest,
  })
  ...
  // - omit by default for lower-friction sign-in
  // - use select_account only when caller explicitly requests it
  if (options.prompt) {
...
export async function exchangeCodeForToken(code, redirectUri = null) {
  requireGoogleConfig({ requireSecret: true, redirectUri })
